...
impl Graph {
    ...
    pub fn insert_neighbor(&mut self, node: NodeId, side: Side) -> NodeId {
        ...
        // guarantees this.
        let shorter_neighbors_of_neighbor = self.populate_shorter_neighbors_of_neighbor(node, side);

        // Select the side along the canonical path from the origin to this node. This is guaranteed
        // to be the first entry of the `shorter_neighbors_of_neighbor` iterator.
        let (parent_side, parent) = shorter_neighbors_of_neighbor.clone().next().unwrap();
        ...
        for (side, neighbor) in shorter_neighbors_of_neighbor {
    ...
    /// Ensure all shorter neighbors of a not-yet-created neighbor node exist, and return them (including the given node)
    /// in the form of ordered pairs containing the side they share with this not-yet-created neighbor node, and their node ID.
    /// These ordered pairs will be sorted by side, based on enum order.
    fn populate_shorter_neighbors_of_neighbor(
        &mut self,
        node: NodeId,
        side: Side,
    ) -> impl Iterator<Item = (Side, NodeId)> + Clone {
        // This function's implementation is highly nontrivial. To help improve readability, we use the term "subject" to refer
        // to the not-yet-created neighbor node.
        ...
        (0..3).filter_map(move |i| shorter_neighbors_of_subject[i])
    }
